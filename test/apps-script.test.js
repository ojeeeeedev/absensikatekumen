...
function load() {
  ...
  const sheet = {
    getLastColumn: () => 2,
    getDataRange: () => ({
      getValues: () => state.presensiRows || [
        Array(12).fill(''),
        ['', 'Ada', '', '', '', '', '', '', '', '', '', '1/SAB/2'],
      ],
    }),
    getRange: vi.fn((row, column) => {
      if (row === 1) return { getValues: () => [['Name', 'Topik R1']] };
      if (column === 12) return { getValue: () => state.rowIds[row] || '' };
      return statusCell;
    }),
...
describe('Apps Script GAS secret contract', () => {
  beforeEach(() => {
    ...
    state.rowIds = { 2: '1/SAB/2' };
    state.presensiRows = null;
  });
  ...
  it('rebuilds a stale row cache before writing attendance', () => {
    state.rowIds = { 2: '1/SAB/999', 3: '1/SAB/2' };
    state.presensiRows = [
      Array(12).fill(''),
      ['', 'Other', '', '', '', '', '', '', '', '', '', '1/SAB/999'],
      ['', 'Ada', '', '', '', '', '', '', '', '', '', '1/SAB/2'],
    ...
    expect(cache.remove).toHaveBeenCalledWith('STUDENT_MAP_V1');
    expect(sheet.getRange).toHaveBeenCalledWith(3, 2);
